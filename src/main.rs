...
fn get_args() -> ArgMatches<'static> {
    let matches = App::new(crate_name!())
        ...
        .arg_from_usage("-M, --send-mail 'Send an email directly from within \
            cwrap itself.  This option is *required* if you are using other \
            SMTP options."
        )
        .arg_from_usage("-N, --suppress-normal-output 'Suppress the normal \
            output to STDOUT.  Generally used with --send-mail."
        )
        .arg(Arg::with_name("email_from")
            .short("-E")
            .long("--email-from")
            .value_name("EMAIL_ADDR")
            .help("The email address to use as the from address")
        )
        .arg(Arg::with_name("email_to")
            .short("-R")
            .long("--email-recips")
            .value_name("EMAIL_ADDRS")
            .use_delimiter(true)  // Default delimiter is a comma
            .help("A comma separated list of email addresses to send to")
        )
        .arg(Arg::with_name("subject")
            .short("-J")
            .long("--email-subject")
            .value_name("STR")
            .default_value("cwrap failure report")
        ...
        .arg(Arg::with_name("smtp_server")
            ...
            .help("The hostname or IP of the email server to connect to")
        )
        ...
        .arg_from_usage("-L, --ssl 'Use SSL for the SMTP server connection")
        .arg_from_usage("-Z, --starttls 'Use STARTTLS for the SMTP server \
            connection")
        .arg(Arg::with_name("smtp_user")
            .short("-U")
            .long("--smtp-username")
            .value_name("USERNAME")
            .help("The username to use for SMTP auth")
        )
        .arg(Arg::with_name("smtp_password")
            .short("-W")
            .long("--smtp-password")
            .value_name("PASSWORD")
            .help("The SMTP password to use for the connection")
        )
        .arg(Arg::with_name("smtp_creds_file")
            .short("-A")
            .long("--smtp-creds-file")
            .value_name("FILE")
            .help("The path to a file containing the SMTP credentials to use \
                for the connection.  This should be a file containing only: \
                USERNAME:PASSWORD"
            )
        )
